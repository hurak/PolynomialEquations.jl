"""
    x = spectralfactor(a[,method=:roots])

Find a spectral factor `x` for a scalar univariate polynomial `a`.

For a scalar univariate polynomial `a` symmetric with respect to the imaginary axis, that is, `a=b̃b`, for some `b`, where `b̃(s)=b(-s)` (for complex polynomials the coefficients are complex-conjugated on top of substituting `-s` for `s`), find a polynomial `x` such that `x̃x=a` and `x` is Hurwitz stable (all roots in the left half plane).
